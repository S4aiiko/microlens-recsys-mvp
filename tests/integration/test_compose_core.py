...
def test_compose_core_has_five_services_and_private_internal_listener() -> None:
    compose = yaml.safe_load((ROOT / "compose.yaml").read_text())
    assert set(compose["services"]) == {"web", "api", "db", "redis", "worker"}
...
def test_live_compose_ready_reports_postgres_alembic_redis_and_restore_boundary() -> None:
    ...
    assert checks["alembic"]["current"] == checks["alembic"]["head"] == "20260901_0003"
